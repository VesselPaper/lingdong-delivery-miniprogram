...
1. **自动派车注释与实现差异**：timers.js 第④扫描头部注释仍描述「批次满 12 单 / 自动接单等待满 90s 自动派车」，当前扫描体仅对 status IN (0,1) 批次做计数与召唤待命，未再调用 doDispatchBatch；实际派车入口为商家手动「派车」按钮与扫码派车（/merchant/device/scan 对组单中批次自动定型）。若产品上仍需「满 12 单自动派车」，需在扫描体内恢复自动派车分支。
