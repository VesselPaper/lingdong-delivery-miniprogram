...
| 页面 | 文档 |
| --- | --- |
| 管理员网页（/admin） | [01_管理员网页.md](管理端与大屏/01_管理员网页.md)（登录机制/导航区块/表格渲染/地图/构建机制） |
| 可视化大屏（/dashboard） | [02_可视化大屏.md](管理端与大屏/02_可视化大屏.md)（布局/数据源/3D 地图/刷新部署） |

## 阅读约定

- 页面数量与路由以两端 `app.json` 实际注册为准（用户端 18 页 + 商家端 22 页，含 tabBar）；
