...
| token 语义 | `server.js → push.attach(validateToken)`：先查 `admin_sessions`（有效未过期且账号启用），再查 `users` 表 `openid=? OR token=?`（用户端 token=openid；商家端账号密码登录签发随机 token 存 users.token）——管理员与用户/商家共用同一通道 |
...
| `batch_dispatched`（全量广播） | `delivery/service.js doDispatchBatch` 定型成功（自动定型/手动上货/扫码定型共用） | `{ "type": "batch_dispatched", "batch_id": 1, "batch_no": "BD-…" }` | 批次自动定型（满容/超 90s）完成 → 商家端配单上货/任务页/工作台重拉，卡面从「组单中」圆环变「待上货」 |
...
客户端约定：商家端收到 `order_created` 或 `batch_dispatched` 后重拉订单列表/工作台统计/批次列表；管理员页收到 `state_changed` 后拉管理员状态总览；断线自动重连（服务端空闲 10 分钟会回收半开连接）。
