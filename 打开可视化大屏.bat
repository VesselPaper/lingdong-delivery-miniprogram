...
curl -s -o nul -m 3 -w "%%{http_code}" "%BASE%/dashboard/" >"%TEMP%\ld_ping.txt" 2>nul
...
echo [提示] 无法连通 %BASE%/dashboard/ (HTTP 状态: %CODE%)
echo   请检查：
...
echo 已在默认浏览器打开可视化大屏 (如未弹出请检查默认浏览器)。
echo 大屏免登录, 可直接全屏展示。
echo.
pause
